...
# 2 a
awk -F'\t' -v max=0 -v id=0 '{if (NR != 1 && max<=(($21/($18-$21))*100)){max=(($21/($18-$21))*100); id=$1}} END{printf("Transaksi terakhir dengan profit percentage terbesar yaitu %d dengan persentase %f\%.\n", id, max)} ' "Laporan-TokoShiSop.tsv" > "hasil.txt"

# 2 b
awk -F'\t' 'BEGIN{printf("\nDaftar nama customer di Albuquerque pada tahun 2017 antara lain:\n")}{if($10 == "Albuquerque") arr[$7]++}END{for (name in arr) {print name}}' "Laporan-TokoShiSop.tsv" >> "hasil.txt"
